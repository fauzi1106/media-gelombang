  ...
  <script>
    ...
    document.addEventListener("change", e => {
      ...
      if (e.target.name === "soal") {
        ...
        raguRagu[currentIndex] = false;   // otomatis hilangkan ragu
        renderNav();
        updateRaguButton();
    ...
    async function finishQuiz(force = false) {
      ...
      if (tuntas) {
        ...
        resultMessage.innerHTML = `Nilai kamu: <b>${nilaiPersen}</b><br>Pelajari ulang materi Gelombang.`;
      ...
      await fetch('/simpan-progress', {
        ...
        body: JSON.stringify({
          halaman: "kuis_gelombang",
          urutan: 6,
          duration: duration
        })
      });

      await fetch("/simpan-nilai", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-CSRF-TOKEN": "{{ csrf_token() }}"
